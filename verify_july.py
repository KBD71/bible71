...
files = [f for f in os.listdir(mccheyne_dir) if f.startswith("mc07") and f.endswith(".html")]
...
for file_name in sorted(files):
    ...
    for dp in data_paths:
        if not dp.startswith("https://kbd71.github.io/bible71/bible_html/"):
            print(f"Error in {file_name}: data-path does not start with correct URL: {dp}")
            errors += 1
        if "markdown" in dp or "[" in dp or "]" in dp:
            print(f"Error in {file_name}: data-path contains markdown or bracket chars: {dp}")
            errors += 1
...
if errors == 0:
    print("All July files are 100% compliant with the premium rules!")
else:
    print(f"Total errors found in July files: {errors}")
